...
The standard prompts project is a set of LLM prompts developed using Claude Opus 4.5 with reasoning and web search enabled.

Prompts were written using the below template to make the requests. When using the template:
1. Replace [technology] with the brief name of the technology.
2. Expand the first use of the term with a more detailed description if needed for clarity.
3. Update the standards list at the end to contain only relevant standards.
...
- **Automation**: When specifying formatting standards, recommend or mandate the use of widely supported automation tools for format checking, style fixing, and linting rather than manually specified rules.
...
Ensure the standards are practical, enforceable, and aligned with official [technology] documentation, for example:
* PSR for PHP
...
Avoid overly rigid rules; prioritize principles that promote sustainable, inclusive software.
